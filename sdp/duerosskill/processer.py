...
from .response import ResponseDUEROS
from .models import *
...
class Processer(object):
    ...
    def __init__(self, req: Dict):
        ...
        self.response.tts_output(HELP_MSG)
        self.request = req
        ...
        (self.device_owner, created) = Dueros_DeviceOwner.objects.get_or_create(userid=uid)
        (self.device, created) = Dueros_Device.objects.get_or_create(
        ...
        if "attributes" in req["session"]:
            for key  in req["session"]["attributes"]:
                self.session_data[key]=req["session"]["attributes"][key]
        '''
        读取原始语句
        '''
    ...
    def launchRequest(self):
        ...
        self.response.tts_output(START_MSG)
        self.gamemode = False
    ...
    def intent_process(self):
        ...
        if self.intent == "ai.dueros.common.default_intent":
            self.response.add_session_data( "gamemode",False)
            self.response.tts_output(AGAIN_MSG)
        elif self.intent == "ai.dueros.common.cancel_intent)":
            self.response.shouldEndSession = True
            self.response.tts_output(EXIT_MSG)
            '''
                自定义意图处理

            '''
        elif self.intent == "gamemode":
            self.response.add_session_data( "gamemode",True)
            self.response.tts_output(GAME_STAET_MSG)
